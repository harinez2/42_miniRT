...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/18 17:39:35 by yonishi           #+#    #+#             */
/*   Updated: 2022/06/18 17:39:36 by yonishi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_vec v_from, t_vec v_to, t_map *m, t_square *ts)
{
	...
	t1 = get_distance_to_triangle(v_from, v_to, m, &ts->tr_a);
	t2 = get_distance_to_triangle(v_from, v_to, m, &ts->tr_b);
	if (t1 > 0 && t2 > 0)
		t = fmin(t1, t2);
	else
		t = fmax(t1, t2);
